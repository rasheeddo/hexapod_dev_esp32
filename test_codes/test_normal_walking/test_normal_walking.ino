...
void setup() {
  ...
  Serial.println("h.normalWalking_PWM_LUT[0][0]");
  print_array(h.normalWalking_PWM_LUT[0][0], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[0][1]");
  print_array(h.normalWalking_PWM_LUT[0][1], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[0][2]");
  print_array(h.normalWalking_PWM_LUT[0][2], DATA_POINT_ALL);
  Serial.println(" ");
  Serial.println("h.normalWalking_PWM_LUT[1][0]");
  print_array(h.normalWalking_PWM_LUT[1][0], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[1][4]");
  print_array(h.normalWalking_PWM_LUT[1][4], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[1][10]");
  print_array(h.normalWalking_PWM_LUT[1][10], DATA_POINT_ALL);
  Serial.println(" ");
  Serial.println("h.normalWalking_PWM_LUT[10][0]");
  print_array(h.normalWalking_PWM_LUT[10][0], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[10][4]");
  print_array(h.normalWalking_PWM_LUT[10][4], DATA_POINT_ALL);
  Serial.println("h.normalWalking_PWM_LUT[10][10]");
  print_array(h.normalWalking_PWM_LUT[10][10], DATA_POINT_ALL);
  Serial.println(" ");
...
void loop() {
  ...
  delay_time = map(sbus_ch[1], sbus_db_max, sbus_max, max_delay, min_delay);
  if (sbus_ch[1] >= sbus_db_max) {

    if ((sbus_ch[0] < sbus_db_max) && (sbus_ch[0] > 1000)) {
      str_index = 7;
    } else if (sbus_ch[0] >= sbus_db_max) {
      str_index = (int)map(sbus_ch[0], sbus_mid, sbus_max, 7, 14);
    } else if (sbus_ch[0] <= sbus_db_min) {
      str_index = (int)map(sbus_ch[0], sbus_min, sbus_mid, 0, 7);
    }
    delay_time = map(sbus_ch[1], sbus_mid, sbus_max, max_delay, min_delay);

  } else if (sbus_ch[1] < sbus_db_min) {

    delay_time = map(sbus_ch[1], sbus_db_min, sbus_min, max_delay, min_delay);
    str_index = 7;
  ...
  if ((period >= delay_time) && ((sbus_ch[1] < sbus_db_min) || (sbus_ch[1] > sbus_db_max)) ) {
  ...
  } else if ((sbus_ch[1] >= sbus_db_min) && (sbus_ch[1] <= sbus_db_max)) {
...
void normalWalking(int ang_inc, int counter) {

  pwm.writeMicroseconds(0, h.normalWalking_PWM_LUT[ang_inc][0][counter]);
  pwm.writeMicroseconds(1, h.normalWalking_PWM_LUT[ang_inc][1][counter]);
  pwm.writeMicroseconds(2, h.normalWalking_PWM_LUT[ang_inc][2][counter]);

  pwm.writeMicroseconds(3, h.normalWalking_PWM_LUT[ang_inc][3][counter]);
  pwm.writeMicroseconds(4, h.normalWalking_PWM_LUT[ang_inc][4][counter]);
  pwm.writeMicroseconds(5, h.normalWalking_PWM_LUT[ang_inc][5][counter]);

  pwm.writeMicroseconds(6, h.normalWalking_PWM_LUT[ang_inc][6][counter]);
  pwm.writeMicroseconds(7, h.normalWalking_PWM_LUT[ang_inc][7][counter]);
  pwm.writeMicroseconds(8, h.normalWalking_PWM_LUT[ang_inc][8][counter]);

  pwm.writeMicroseconds(9, h.normalWalking_PWM_LUT[ang_inc][9][counter]);
  pwm.writeMicroseconds(10, h.normalWalking_PWM_LUT[ang_inc][10][counter]);
  pwm.writeMicroseconds(11, h.normalWalking_PWM_LUT[ang_inc][11][counter]);

  pwm.writeMicroseconds(12, h.normalWalking_PWM_LUT[ang_inc][12][counter]);
  pwm.writeMicroseconds(13, h.normalWalking_PWM_LUT[ang_inc][13][counter]);
  pwm.writeMicroseconds(14, h.normalWalking_PWM_LUT[ang_inc][14][counter]);

  pwm.writeMicroseconds(15, h.normalWalking_PWM_LUT[ang_inc][15][counter]);
  pwm_2.writeMicroseconds(0, h.normalWalking_PWM_LUT[ang_inc][16][counter]);
  pwm_2.writeMicroseconds(1, h.normalWalking_PWM_LUT[ang_inc][17][counter]);
